...
def test_health_check():
    ...
    assert response.json() == {"message": "This is an API health check successful: status successful"}
...
def test_list_players_by_name():
    ...
    players_response = client.list_players(
        first_name="Bryce", last_name="Young")
...
def test_bulk_player_file():
    ...
    assert rows[0] == ['player_id', 'gsis_id', 'first_name',
...
def test_bulk_league_file():
    ...
    # Additional check: ensure the first row is the header
    assert rows[0] == ['league_id', 'league_name',
...
def test_bulk_performance_file():
    ...
    # Additional check: ensure the first row is the header
    assert rows[0] == ['performance_id', 'week_number',
                       'fantasy_points', 'player_id', 'last_changed_date']
...
def test_bulk_team_file():
    ...
    # Additional check: ensure the first row is the header
    assert rows[0] == ['team_id', 'team_name',
                       'league_id', 'last_changed_date']
